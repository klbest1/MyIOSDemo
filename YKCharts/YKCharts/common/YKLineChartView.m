...
-(void)installDrawlines{
    for (NSInteger i = 0 ; i < _dataSource.showNumbers.count; i ++){
        ...
        //计算 y 的位置
        CGFloat percentage = (1 - (number-_dataSource.min)/(_dataSource.max - _dataSource.min));
...
-(void)touchAtIndex:(NSInteger)index{
    ...
    CGRect screenRect = [_drawLineView convertRect:drawRect toView:[UIApplication sharedApplication].keyWindow];
...
#define USE_CUSTOM_SETTINGS
    ...
    [FTPopOverMenu showFromSenderFrame:rect withMenuArray:@[content]  doneBlock:^(NSInteger selectedIndex) {
