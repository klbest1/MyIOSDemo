...
+ (void) showFromSenderFrame:(CGRect )senderFrame
                   superView:(UIView *)superView
                    instance:(FTPopOverMenu *)instance
               withMenuArray:(NSArray *)menuArray
                   doneBlock:(FTPopOverMenuDoneBlock)doneBlock
                dismissBlock:(FTPopOverMenuDismissBlock)dismissBlock
{
    
    [instance showFromView:superView senderFrame:senderFrame withMenu:menuArray imageNameArray:nil doneBlock:doneBlock dismissBlock:dismissBlock];
}
...
//        [self.backgroundView addSubview:self.popMenuView];
        [[self backgroundWindow] addSubview:self.popMenuView];
...
- (void) showFromView:(UIView *)supview
           senderFrame:(CGRect )senderFrame
              withMenu:(NSArray *)menuArray
        imageNameArray:(NSArray *)imageNameArray
             doneBlock:(FTPopOverMenuDoneBlock)doneBlock
          dismissBlock:(FTPopOverMenuDismissBlock)dismissBlock
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        [supview addSubview:self.popMenuView];
        
        self.superView = supview;
        self.senderFrame = senderFrame;
        self.menuArray = menuArray;
        self.menuImageArray = imageNameArray;
        self.doneBlock = doneBlock;
...
-(void)adjustPopOverMenu
{
    ...
    if ([FTPopOverMenuConfiguration defaultConfiguration].showCustomDirection) {
        arrowDirection = [FTPopOverMenuConfiguration defaultConfiguration].showDirection;
    ...
    _popMenuView.userInteractionEnabled = [FTPopOverMenuConfiguration defaultConfiguration].enableUserInteraction;
...
-(void)doneActionWithSelectedIndex:(NSInteger)selectedIndex
{
    [UIView animateWithDuration:FTDefaultAnimationDuration
                     animations:^{
                         ...
                         if (finished) {
                             [self->_popMenuView removeFromSuperview];
